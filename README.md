...
This repo is the shelf plus a static split-screen arcade pack. The original
GrauNinja and Grau Battle entries still point to their standalone game repos;
the 20 new classic-game remixes share the lightweight `_duel/` runtime and are
selected by a query parameter, so each shelf card still opens its own themed
game screen without a build step.

```
graugames.github.io/            <- this repo (the shelf)
graugames.github.io/grau-ninja/ <- repo: graugames/grau-ninja
graugames.github.io/_duel/      <- shared runtime for the classic remixes
```
...
| `_duel/` | themed 1v1 canvas games with keyboard, pointer, touch and optional hand tracking |
| `404.html` | not-found page, same styling |
| `og.png` | 1200x630 social preview, generated by `tools/make-og.py` |

There is no build step. Pages serves these files exactly as they are — which is
also why `og.png` is committed rather than generated on deploy. Re-run
`python tools/make-og.py` if the wordmark ever changes.
